...
const webhookTranscript = true; // if send important logs to webhook logs IMPORTANT: ONLY WORKS FOR WARN AND ERROR IF WANT OTHER LOGS BE LOGGED NEED TO EDIT THE CODE DM ME FOR HELP (^_−)−☆
const webhookTranscriptTokens = {
    error: {
        id: '983232403260723211', // the webhook id
        token: 'cn-Mf3X582IJANp1klQKO8DVQeljvs5L_fbbna87dRkxKVZtTGrgWV687wMIocjKftEv', // the webhook token
    },
    warn: {
        id: '983232437771460638', // the webhook id
        token: 'JxnlXgAg6kLv35SIeQsWZVRXADgGVV3qmgDPeoQAycTEmflb6lSfOkbC8Kr1ntaE7hzi', // the webhook token
    },
};
const Colors = {
...
exports.log = async (msg, type = 'log') => {
    ...
    if (msg === '' || typeof msg !== 'string')
        throw new Error('[Logger] Invalid Logging Content.'); // check if msg is a string and not em-pty
    if (!logTypes.includes(type.toLowerCase()))
        throw new Error('[Logger] Invalid Logging Type.'); // check if logger is valid
    ...
    function _filename() {
        ...
        filename =
            `[` +
            filename.split(/[\\/]/).pop() + // if want the full path delete this
    ...
    function transcriptToWebhook() {
        ...
        if (type === 'error' || type === 'warn') {
            ...
            const webhook_sender = new Discord.WebhookClient({
                id:
                    type === 'warn'
                        ? webhookTranscriptTokens.warn.id
                        : webhookTranscriptTokens.error.id,
                token:
                    type === 'warn'
                        ? webhookTranscriptTokens.warn.token
                        : webhookTranscriptTokens.error.token,
    ...
    console.log(
        `${Colors.timestamp}[${dateNow}]${Colors.reset} - ${
            Colors.logType
        }[${type}]${Colors.reset} ~ ${Colors.fileName}${_filename()}${
            Colors.reset
        } ${log_color()}${msg}${Colors.reset}`
    );
    transcript();
    transcriptToWebhook();
};
